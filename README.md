# Modifications by TUFIQoE
## user_id
User ID is not generated by the software but has to be provided by the experiment administrator. 
There are two welcome screens rendered in browser: default welcome and training welcome. 
Additional cookie "training_enabled" was implemented. Based on its value "true" or "false" input field 
for the user ID is generated in training_welcome or in welcome. 

Enabling and disabling training session is done in config.json file.

### ### Important ### ####
When switching between experiment with/without training session clear all of the browser cookies related to the 
avrate local backend server. 

Also backend software has to be restarted in order to apply changes that affect browsers cookies.
